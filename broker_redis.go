...
func (b *RedisBroker) checkCapabilities(shard *RedisShard) error {
	...
	dr := shard.newDataRequest("XRANGE", nil, "", nil, []interface{}{"_", "0-0", "0-0"})
...
func (b *RedisBroker) removeHistory(s *RedisShard, ch string) error {
	...
	dr := s.newDataRequest("DEL", nil, key, nil, []interface{}{key})
	resp := s.getDataResponse(dr)
	return resp.err
}
...
func (b *RedisBroker) runPubSub(s *RedisShard, eventHandler BrokerEventHandler) {
		...
		go func(ch chan *redis.Message) {
			for {
				...
				case n := <-ch:
					...
					default:
						err := b.handleRedisClientMessage(eventHandler, channelID(n.Channel), []byte(n.Payload))
						if err != nil {
	...
	for {
		...
		if err != nil {
			b.node.Log(NewLogEntry(LogLevelError, "Redis receiver error", map[string]interface{}{"error": err.Error()}))
			s.reloadPipeline()
			return
		}
		switch m := m.(type) {
		case *redis.Message:
			...
			workers[index(m.Channel, numWorkers)] <- m
		default:
		}
	}
}
			...
			for {
				...
				case n := <-workCh:
					...
					case pingChannel:
						// Do nothing - this message just maintains connection open.
					default:
						err := eventHandler.HandleControl([]byte(n.Payload))
						if err != nil {
...
func (b *RedisBroker) runPubSubPing(s *RedisShard) {
	...
	for {
		...
		cmd := s.client.Publish(context.Background(), b.pingChannel, nil)
		if cmd.Err() != nil {
...
func (b *RedisBroker) runPublishPipeline(s *RedisShard) {
	...
	loop:
		...
		cmds, err := s.client.Pipelined(context.Background(), func(pipe redis.Pipeliner) error {
			for i := range prs {
				_ = pipe.Publish(context.Background(), string(prs[i].channel), prs[i].message)
			}
			return nil
		})
		if err != nil {
